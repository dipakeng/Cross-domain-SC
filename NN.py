...
class NN:
    ...
    def train_1(self, X, y, eta=0.5, n_epochs=200):
        for epoch in range(n_epochs):
            print(epoch)
            for (x_, y_) in zip(X, y):
                self._forward_pass(x_) # forward pass (update node["output"])
                yhot_ = self._one_hot_encoding(y_, self.output_dim) # one-hot target
                self._backward_pass(yhot_) # backward pass error (update node["delta"])
                self._update_weights_1(x_, eta) # update weights (update node["weight"])
    def train_2(self, X, y, eta=0.5, n_epochs=200):
        for epoch in range(n_epochs):
            print(epoch)
            for (x_, y_) in zip(X, y):
                self._forward_pass(x_) # forward pass (update node["output"])
                yhot_ = self._one_hot_encoding(y_, self.output_dim) # one-hot target
                self._backward_pass(yhot_) # backward pass error (update node["delta"])
                self._update_weights_2(x_, eta) # update weights (update node["weight"])
        ...
        for i, layer in enumerate(self.network):
            ...
            for node in layer:
                for j, input in enumerate(inputs):
                    ...
                    # if j>3:# modification
                    #     node['weights'][j]=(node['weights'][j]+node['weights'][j-1]+node['weights'][j-2]+node['weights'][j-3])/4

    def _update_weights_1(self, x, eta):
        for i, layer in enumerate(self.network):
            # Grab input values
            if i == 0:
                inputs = x
            ...
            for node in layer:
                for j, input in enumerate(inputs):
                    ...
                    node['weights'][j] += - eta * node['delta'] * input
                    if j > 3:  # modification
                        node['weights'][j] = (node['weights'][j] + node['weights'][j - 1] + node['weights'][j - 2]) / 3

    def _update_weights_2(self, x, eta):
        for i, layer in enumerate(self.network):
            # Grab input values
            if i == 0:
                inputs = x
            ...
            for node in layer:
                for j, input in enumerate(inputs):
                    ...
                    if j > 3:  # modification
                        r1=0.25
                        r2=0.75
                        XN=node['weights'][j - 3]+r1*(node['weights'][1]-node['weights'][j])+r2*(node['weights'][j-1]-node['weights'][j])
                        node['weights'][j] = (node['weights'][j] + node['weights'][j - 1] + node['weights'][j - 2] +
                                              node['weights'][j - 3]+XN) / 4
